...
def predict():
    ...
    features=np.array([[bill_length,bill_depth,flipper_length,body_mass,sex,island]])
    prediction=model.predict(features)
    species=encoder.inverse_transform(prediction)
    return render_template("random.html",prediction_text=f"Predicted Penguin Species: {species[0]}")
